...
with tabs[3]:
    ...
    for col, brand in zip(kpi_cols, brands_bp):
        ...
        col.markdown(
            ...
            f'均价 ৳{avg_price:,.0f} ｜ 含冰 {ice_pct:.0%} ｜ 复合口味 {multi_pct:.0%}<br>'
...
with tabs[6]:
    ...
    if price_df.empty:
        ...
        for col, dev in zip(price_cols, fdf["设备类别"].dropna().unique()):
            ...
            if not sub.empty:
                col.markdown(
                    f'<div class="metric-card"><div class="value">৳{sub.mean():,.0f}</div>'
                    f'<div class="label">{dev} 均价<br>（৳{sub.min():,.0f}—৳{sub.max():,.0f}）</div></div>',
                    unsafe_allow_html=True
                )
        st.markdown("<br>", unsafe_allow_html=True)
        ...
        with col_l:
            ...
            st.caption(f"一次性均价 ৳{disp_avg:,.0f}，烟油均价 ৳{liq_avg:,.0f}；一次性产品价格带更集中，竞争更激烈。")
        ...
        with col_r:
            ...
            top_cp = cp.iloc[0]
            st.caption(f"{top_cp['品牌来源国']}品牌均价最高（৳{top_cp['价格_数值']:,.0f}）；各国品牌价格差异明显，消费者可选择空间较大。")
        ...
        if not bp_price.empty:
            ...
            fig_bpb = px.bar(brand_price_bp, x="品牌", y="均价",
                             ...
                             template=T)
            fig_bpb.update_traces(texttemplate="৳%{text:,.0f}", textposition="outside")
            ...
            st.caption(
                f"Elf Bar 均价 ৳{eb_avg:,.0f} vs Voltbar 均价 ৳{vb_avg:,.0f}；"
...
with tabs[7]:
    ...
    st.markdown("- **定价策略**：参考 ৳1,650–2,000 价格带，对标 Elf Bar / Voltbar，走量先于溢价。")
    st.markdown('</div>', unsafe_allow_html=True)